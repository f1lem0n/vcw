...
def get_authenticated_service(args):
    ...
    storage = Storage(
        "%s-oauth2.json"
        % str(Path(args.secrets_path).absolute() / args.yt_account)
    )
...
if __name__ == "__main__":
    argparser.add_argument(
        "--yt_account", required=True, help="YouTube account name"
    )
    argparser.add_argument(
    ...
    CLIENT_SECRETS_FILE = str(
        Path(args.secrets_path).absolute() / "client_secrets.json"
    )
